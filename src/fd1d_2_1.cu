...
void allocateOnHost(FDTDData& host, std::size_t dim)
{
	host.e = (double*)malloc(dim * sizeof(double));
	host.h = (double*)malloc(dim * sizeof(double));
	host.aux = (double*)malloc(2 * sizeof(double));
	host.gax = (double*)malloc(dim * sizeof(double));
	host.gbx = (double*)malloc(dim * sizeof(double));
	host.dx = (double*)malloc(dim * sizeof(double));
	host.ix = (double*)malloc(dim * sizeof(double));
}

void allocateOnDevice(FDTDData& dev)
{
	cudaMalloc(dev.e, dim * sizeof(double));
	cudaMalloc(dev.h, dim * sizeof(double));
	cudaMalloc(dev.aux, 2 * sizeof(double));
	cudaMalloc(dev.gax, dim * sizeof(double));
	cudaMalloc(dev.gbx, dim * sizeof(double));
	cudaMalloc(dev.dx, dim * sizeof(double));
	cudaMalloc(dev.ix, dim * sizeof(double));
}
